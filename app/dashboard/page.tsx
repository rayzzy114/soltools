              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    className="h-5 px-2 text-[10px] text-blue-500 hover:text-blue-600"
                    onClick={() => {
                      if (!connectedWalletKey) {
                        toast.error("Connect wallet first")
                        return
                      }
                      const match = bundlerWallets.find((w) => w.publicKey === connectedWalletKey)
                      if (match && match.secretKey) {
                        setLaunchDevWallet(match.publicKey)
                        toast.success("Connected wallet selected")
                      } else {
                        toast.error("Connected wallet not found in saved wallets (import it first)")
                      }
                    }}
                  >
                    Use connected
