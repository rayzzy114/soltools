...
def run(playwright):
    ...
    if page.get_by_role("button", name="Open main stage").is_visible():
        page.get_by_role("button", name="Open main stage").click()

    # Wait for stage change
    page.wait_for_timeout(1000)

    # Check if we are in main stage (Look for "VOLUME BOT" text which is in main stage)
    # page.wait_for_selector("text=VOLUME BOT")

    # Locate wallet in grid. The mock has "Wallet 1".
    # The WalletRow renders "1. Wallet 1"
    wallet_locator = page.locator("button").filter(has_text="Wallet 1")
    wallet_locator.first.wait_for(state="visible", timeout=10000)
    wallet_locator.first.click()

    # Wait for dialog
    page.wait_for_selector("text=Wallet Trade")

    # Take screenshot
    page.screenshot(path="verification/dialog_screenshot.png")
